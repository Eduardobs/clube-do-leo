class ProductManager {
  ...
  async loadProducts() {
    ...
    try {
      ...
      this.products.forEach(product => {
        if (!product.id || !product.nome || !product.valor || !Array.isArray(product.variacoes)) {
  ...
  showProductDetail(productId) {
    ...
    detailSection.innerHTML = `
      ...
      <button class="btn" onclick="cartManager.addToCart('${product.id}', document.getElementById('variation-${product.id}').value, document.getElementById('quantity-${product.id}').value); productManager.closeProductDetail()">Adicionar ao Carrinho</button>
    `;
