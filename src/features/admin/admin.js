...
class AdminManager {
  ...
  bindEvents() {
    ...
    this.el.imageList.addEventListener('input', (e) => {
      ...
      if (input) input.nextElementSibling.src = input.value || 'assets/brand/header-logo-medallion.png';
    });
  ...
  async loadInitial() {
    ...
    if (draft) {
      ...
      this.products = this.normalizeProducts(keepDraft ? draft : fetched || []);
      if (!keepDraft) this.clearDraft();
    } else {
      this.products = this.normalizeProducts(fetched || []);
    }
  ...
  normalizeProducts(products) {
    return products.map(({ subcategorias, ...product }) => {
      const categorias = (product.categorias || []).filter((categoria) => CONFIG.categorias.includes(categoria));
      return { ...product, categorias: categorias.length ? categorias : ['Jogos'] };
    });
  }
  ...
  renderTable() {
    ...
    this.el.tableBody.innerHTML = this.filtered
      .map((product) => {
        const thumb = product.imagens?.[0] || 'assets/brand/header-logo-medallion.png';
        const categorias = (product.categorias || [])
          .map((c) => `<span class="badge">${Utils.escapeHtml(c)}</span>`)
          .join('');
        return `
        <tr>
          <td><img class="admin-table__thumb" src="${Utils.escapeHtml(thumb)}" alt="${Utils.escapeHtml(product.nome)}" onerror="this.src='assets/brand/header-logo-medallion.png'"></td>
  ...
  importJson(file) {
    ...
    reader.onload = () => {
      try {
        ...
        this.products = this.normalizeProducts(data.produtos);
        this.saveDraft();
        this.applyFilter();
